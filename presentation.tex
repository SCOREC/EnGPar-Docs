...
\begin{frame}[plain]
  \frametitle{Session}
  \url{http://meetings.siam.org/sess/dsp_programsess.cfm?SESSIONCODE=63562}
  \begin{itemize}
    \item 2:40-3:00 ``Underappreciated Bottlenecks for Graph Algorithm
      Communication", Scott Beamer, Lawrence Berkeley National Laboratory, USA
    \item 3:05-3:25 ``SlimSell: A Vectorized Graph Representation for
      Breadth-First Search", Torsten Hoefler, ETH Zürich, Switzerland
    \item 3:30-3:50 ``Triangle Counting and Truss Decomposition on Modern Parallel
      Architectures", Ancy Tom and Shaden Smith, University of Minnesota, USA; George Karypis,
      University of Minnesota and Army HPC Research Center, USA
    \item 3:55-4:15 ``Dynamic Load Balancing of Massively Parallel Graphs for Scientific
      Computing on Many Core and Gpu Based Systems", Cameron Smith, Gerrett
      Diamond, George M. Slota, and Mark Shepard, Rensselaer Polytechnic
      Institute, USA
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item General implementation sends/receives vertices and their adjacent
      hyperedges to/from any neighbor - communications on host
    \item Rebuild hypergraph after communications complete
  \end{itemize}

  \begin{figure}
    \centering
    \includegraphics[width=.4\textwidth]{results/aero1Belm/migration.png}
    \caption{1B elm mesh balanced on 128Ki, 256Ki, and 512Ki with EnGPar}
  \end{figure}
