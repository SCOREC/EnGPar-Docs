...
\title[Accelerating  Dynamic Load Balancin]{
...
\author[C. Smith]{\underline{Cameron W. Smith}, Gerrett Diamond, Lucas Davis, Mark S. Shephard}
...
\section{Accelerating Selection Performance}
\begin{frame}
  \frametitle{}
  \center \huge{Accelerating Selection Performance}
\end{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        ...
        \item Programming models and languages: OpenCL, Xilinx Vivado HLS,
          OpenARC, initial OpenMP support
        \item Long pipelines yield highest performance; data parallel can
          also be effective
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
...
\begin{frame}
  \frametitle{Selection Initial Results}
  RESULTS HERE ON UPRIGHT MESHES USED FOR BFS\\
  COMPARE WITH SERIAL QUALITY AND TIME
\end{frame}

\begin{frame}
  \frametitle{FPGA: Pipelining Graph and Mesh Operations}
  Static graph: CSR \\
  Dynamic graph: collecting adjacencies, topology modifications (e.g., migration, adapt) \\
  One approach: Array of Sliding Windows (AOSW) + degree list $\rightarrow$ CSR\\
  Example: inversion of regular graph to graph - FIGURE
  \begin{itemize}
    \item read child $\rightarrow$ get parent index $\rightarrow$ insert parent at head of child adjacency array $\rightarrow$ shift adjacency array
    \item prefix sum of degree list to compute offsets
    \item shift entries left to fill gaps
  \end{itemize}
  Replacing adjacency array with a sliding window reduces initialization interval from 15 cycles to XYZ cycles - each iteration a new edge starts getting processed
\end{frame}
...
\begin{frame}
  ...
  \begin {itemize}
  \item Reduces the high mesh vertex imbalance from the 512Ki mesh from 1.53 to 1.13.
...
\begin{frame}
  ...
  \begin{itemize}
    \item Sub-graph intersection and union
  \end{itemize}
