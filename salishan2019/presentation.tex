...
\begin{frame}
  \frametitle{XGC Developments at SCOREC ?BETTER TITLE?}
  \begin{columns}
    \begin{column}{0.5\textwidth}
...
\begin{frame}
  ...
  The Center for Efficient Exascale Discretizations (CEED) is a co-design center
  within the DOE  Exascale Computing Project (ECP)
  \begin{columns}
    \begin{column}{0.5\textwidth}
      Goals
      \begin{itemize}
    ...
    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \includegraphics[width=.7\textwidth]{figures/laghosShockTriplePt.png} \\
        \tiny{The Laghos miniapp solves the time-dependent Euler equation of compressible gas
        dynamics in a moving Lagrangian frame using high-order finite element
...
\begin{frame}
  \frametitle{MFEM Laghos - Early Sierra results (V. Tomov)}
  \includegraphics[width=0.4\textwidth]{figures/blast_3D_rates.png} \\
  Strong scaling for the Triple Point problem in BLAST.
  Both plots compare partial assembly (PA) vs full assembly (FA).
  PA wins for orders above 2.
\end{frame}
