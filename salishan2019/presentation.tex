...
\begin{frame}
  ...
  \end{itemize}
  Inter-GPU data movement is expensive. Some possible load balancing mitigations:
  \begin{itemize}
  ...
  \end{itemize}
  Suppose we had a processor with multiple specialized accelerators
  (e.g., compression, fft, spmv, graph traversal, encryption, etc...)
  \begin{itemize}
    \item Will OpenMP, OpenCL, Kokkos, Raja, etc.. work effectively?
    \item Can the current application code be refactored or will major pieces
      require a rewrite?
    \item Can we still use message passing to coordinate nodes?
  \end{itemize}
\end{frame}
