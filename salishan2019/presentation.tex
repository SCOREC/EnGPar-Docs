...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item Inter-node EDR Infiniband - 25GBs
        \item Intra-node GPU-GPU and CPU-GPU NVLink - 50GBs
        \item Intra-GPU HBM - 900GBs
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \includegraphics[width=.4\textwidth]{figures/summit-node.png}\\
...
\begin{frame}
  ...
  Courtesy of Eric Nielsen and Aaron Walden at NASA Langley.
  \begin{columns}
    \begin{column}{0.5\textwidth}
      Goals
      \begin{itemize}
...
\begin{frame}
  \frametitle{FUN3D - Finite volume CFD (cont.)}
  \begin{figure}
    \centering
    \includegraphics[width=.9\textwidth]{figures/fun3d-summit.png}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item place processes near those they share domain data with
    \item optimizing partitions for communication, possibly sacrificing load imbalance
    \item buffering and compressing messages
    \item overlapping communication with computation - on-node tasking?
  \end{itemize}
