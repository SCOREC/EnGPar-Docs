...
\end{frame}
        ...
        %\begin{tabular}{lllll}
        %System            & Summit  & Summit  & Stampede2 & Mira \\
        %                  & (inter) & (intra) &           & \\
        %Stream  (GB/s)    & 5130    & 855     & 194       & 27 \\
        %Network BW (GB/s) & 25      & 50      & 12        & 20 \\
        %Stream/Network    & 205     & 17      & 17        & 1.4
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      Models the whole-volume %, in contact w/ material wall, and  includes heat, momentum and particle sources
      \begin{itemize}
	\item Study non-Maxwellian plasma
	\item Monte-Carlo neutral recycling with atomic data
	\item Fully nonlinear Fokker-Planck collision operation
	\item Multiple impurity species
      \end{itemize}
      Study overlapping multiscale, \\ % multi physics in space and time w/o the usual perturbative approach \\
      Highly compute-intensive per simulation time step \\
      Difficult edge geometry with magnetic %X-point and must scale well on leadership class computers
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
