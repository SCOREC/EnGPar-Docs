...
%If I can add logic in to improve the edge cut increase by using ghost degrees we can use the point being made above.
...
Currently, partitioning in FUN3D is done by running (Par)METIS on the graph created
from mesh vertices and mesh edges ({\color{red} Double check this. It might be with
mesh elements}). As normally seen from multilevel graph partitioning [REFERENCES],
the vertices are partitioned well, but the mesh edges and mesh elements have a large
imbalance which are key components of the computational load in FUN3D.
